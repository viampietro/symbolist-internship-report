...
% Dans \textit{symbolist}, \textit{y = 0} correspond à la ligne délimitant le haut de la fenêtre graphique. De plus, l'échelle de mesure est inversée, c'est à dire un point de coordonnées $(x_1, y_1)$ avec $y_1 = 3$, est représenté verticalement plus bas, sur une partition \textit{symbolist}, qu'un point $(x_2, y_2)$ avec $y_2 = 2$.

\begin{procedure}[H]
	\caption{ApplyKey($staff$, $clef$)}
	\label{alg:applyKey}
    
    \SetKwFunction{clef}{Clef}
	...
	\KwData{$clef$ une fonction prenant en paramètre une variable de type \texttt{struct Symbol} et retournant une valeur de type \textit{float}}
    \KwResult{La valeur de l'attribut $y$ de chacun des symboles du $staff$ est mise à jour par la valeur retournée par la fonction $clef$}
	
    \DontPrintSemicolon
    \BlankLine
    
    	\tcp{$symbol.y$ est équivalent à la syntaxe $y(symbol)$}
    	\ForEach {$symbol$ t.q $symbol \in staff$}{
    		$symbol.y = clef(symbol)$\;
    	}
...
Ici la clef calcule la norme du vecteur \textit{color} pour chaque symbole et utilise cette valeur pour les ordonner verticalement.
